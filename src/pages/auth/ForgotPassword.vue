...
import { sendPasswordResetLink } from '../../utils/SendActivationLink';
...
export default {
  ...
  methods:{
    async resetPassword(event:Event){
      ...
      let otp:number = generateOTP(6);
      const serverIp = window.location.origin;
      this.isLoading =true;
      try {
        const data = {"email": this.email, "otp": otp};
        let res = await axios.put(server+"/reset-password",data)
        if (res.status===200) {
         let res:any =await sendPasswordResetLink(this.email,serverIp,encrypt(this.email),encrypt(otp.toString()));
         if (res.status ===200) {
          this.isLoading = false;
            this.$router.replace({name:"accountRegistered"})
         }
         else{
          this.error="Sorry, we're unable to complete your password reset request at this time."+
         "Please try again later or reach out to our support team for assistance";
        this.isLoading=false; 
        }
        }
      } catch (error) {
...
<template>
  <div
    ...
    <div class="w-full md:w-1/2 flex flex-col items-center bg-white py-5 md:py-8 px-4">
      ...
      <form  @submit="resetPassword" class="px-3 flex flex-col justify-center items-center w-full gap-3 ">
